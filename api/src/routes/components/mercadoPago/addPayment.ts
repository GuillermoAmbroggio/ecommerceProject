...
const { MP_ACCESS_TOKEN, MP_PUBLIC_API_KEY, MP_URL } = process.env;
  ...
  async (req: Request<{}, {}, ReqBodyAddPayment>, res, next) => {
    try {
      ...
      if (mp_user) {
        ...
        axios
          .post(
            `${MP_URL}/card_tokens?public_key=${MP_PUBLIC_API_KEY}&locale=en&js_version=2.0.0`,
            {
              card_number,
              cardholder,
              expiration_month,
          ...
          .then(({ data: getToken }) => {
            ...
            axios
              .post(
                `${MP_URL}/customers/${mp_user.customer_id}/cards`,
                {
                  token: getToken.id,
                },
                configMpHeader
        ...
        if (dataUser) {
          ...
          axios
            .get(`${MP_URL}/customers/search?email=${email}`, configMpHeader)
            .then(({ data: userMP }) => {
              if (userMP.results.length) {
                ...
                axios
                  ...
                  .then(({ data: getToken }) => {
                    ...
                    axios
                      .post(
                        `${MP_URL}/customers/${userMP.results[0].id}/cards`,
                        {
                          token: getToken.id,
                        },
                        configMpHeader
                  ...
                  .then(({ data: createCustomer }) => {
                    ...
                    axios
                      ...
                      .then(({ data: getToken }) => {
                        ...
                        axios
                          .post(
                            `${MP_URL}/customers/${createCustomer.id}/cards`,
                            {
                              token: getToken.id,
                            },
                            configMpHeader
